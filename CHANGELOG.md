...
- **Linter packaged into the plugin** — `build-plugin.sh` now copies `tools/spec-linter/` into `plugin/tools/spec-linter/` (excluding `.venv`, `tests/`, and caches), rewrites `tools/spec-linter/` references to `${CLAUDE_PLUGIN_ROOT}/tools/spec-linter/`, and restores the wrapper's executable bit, so the workflow agents' phase-document checks can run inside an installed plugin.
- **Loud-degrading consumer bindings in the workflow agents** — the Define and Design agents now run the `spec-lint` wrapper against the document they produce and branch on the exit code (0 proceed, 1 block-and-fix, 2 record a VISIBLE `⚠️ contract check skipped` note and proceed — never an assumed PASS). Brainstorm, Build, Ship, and Iterate are marked accurately: only Define and Design have `required_sections` today, so the document-level check is N/A for the others until phase contracts are added.
