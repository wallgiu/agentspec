...
2. **Placement.** Three tiers:
   - Distributed skill → `${CLAUDE_PLUGIN_ROOT}/skills/<name>/`; the build mirrors it into `plugin/skills/`.
   - Plugin-only skill → `plugin-extras/skills/<name>/`; merged in at build, absent from `.claude/`.
   - Repo-local skill (for contributors of this repository, not distributed) → `${CLAUDE_PLUGIN_ROOT}/skills/<name>/` plus an entry in the `REPO_LOCAL_SKILLS` exclusion list in `build-plugin.sh`.
...
4. **Internal paths.** Reference `${CLAUDE_PLUGIN_ROOT}/kb/...`, `${CLAUDE_PLUGIN_ROOT}/skills/...`, `${CLAUDE_PLUGIN_ROOT}/agents/...` normally — the build rewrites them to plugin paths. `.claude/sdd/` workspace paths (`drafts/`, `features/`, `reports/`, `archive/`) are preserved as-is by design.
